    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-5xl">
            {departments.map(departemen => (
                <Card key={departemen.id} className="hover:border-primary transition-colors cursor-pointer flex flex-col h-full" onClick={() => handleSelect(departemen.id)}>
                    <CardHeader>
                        <div className="p-3 bg-primary/10 w-fit rounded-lg mb-4">
                            <GraduationCap className="w-6 h-6 text-primary" />
                        </div>
                        <CardTitle className="text-xl">{departemen.name}</CardTitle>
                        <CardDescription>Kode: {departemen.code}</CardDescription>
                    </CardHeader>
