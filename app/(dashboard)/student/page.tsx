...
export default async function MahasiswaDashboard() {
    ...
    const coursesResponse = await getStudentCourses(mhsUser.id, mhsUser.activeDepartmentId)
    const activeCourses = (coursesResponse.success && coursesResponse.enrollments) ? coursesResponse.enrollments : []

    // Fetch real metrics
    const metricsRes = mhsUser ? await getStudentDashboardMetrics(mhsUser.id, mhsUser.activeDepartmentId) : null
